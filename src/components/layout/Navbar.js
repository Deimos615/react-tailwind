...
const Navbar = ({logo}) => {
  ...
  return (
    <div className="flex flex-row absolute w-full top-0 items-center bg-scroll justify-between h-20 sm:px-vw7">
      <Link to="/" className=" flex items-center pl-7 logo">
        <img height="64" width="59" className=" rounded-lg" src={logo} alt="logo" />
        <span className="hidden sm:block font-sans font-bold text-black text-2.5vw ml-2vw">Due Diligenze</span>
      </Link>

          {/* Mobile Nav */}

          <a onClick={()=>setOpenNav(true)} className="absolute right-5 sm:right-14 hamburger lg:invisible">
          ...
          {
             openNav &&
              <div ref={wrapperRef} className="absolute top-0 w-full h-48 bg-white z-10 nav_border">
                ...
                <div className="flex flex-col items-center justify-around h-full pt-5">
                  <Link onClick={()=>setOpenNav(false)} to="/promotion" className="text-lg font-semibold text-blue-800 transition-all hover:text-orange-500 nav_item_border w-full text-center pb-4">Promotion</Link>
                  ...
                  <Link onClick={()=>setOpenNav(false)} to="/recruitment" className="text-lg font-semibold text-blue-800 transition-all hover:text-orange-500 nav_item_border w-full text-center pb-4">Recruitment</Link>
                  <Link onClick={()=>setOpenNav(false)} to="/download" className="text-lg font-semibold text-blue-800 transition-all hover:text-orange-500 w-full text-center pb-4">Download</Link>
                </div>
              </div>
          }
          ...
          <div className="flex justify-around w-2/5 sm:mr-12 lg:visible invisible">
